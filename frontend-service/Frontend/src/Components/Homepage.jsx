...
const Homepage = () => {
  ...
  const handleSeeSeats = (train) => {
    const formattedDate = new Date(train.date).toISOString().split('T')[0]; // Ensure the date is in 'YYYY-MM-DD' format
  ...
  const handleSubmit = (e) => {
    ...
    fetch('http://localhost:8081/trains/search', {
      ...
      .then(data => {
        ...
        // Pass train data to the trains page
        // Pass the entire formData as well, which includes trainId, date, etc.
        navigate('/trains', { state: { trainData: data, formData } });
      })
